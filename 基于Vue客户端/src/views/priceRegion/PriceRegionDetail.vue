<template>
    <div id="priceRegion">
        <nav-bar class="nav">
            ...
            <template v-slot:center>价格区间详情</template>
        ...
        <scroll class="content" top="1" bottom="50" ref="scroll" style="padding: 0px 5px">
            <div class="main-box">
                <div class="info-bar">
                    <span class="title2">&nbsp;区间id：</span>
                    <span class="title3">{{regionId}}</span>
                </div>
                <div class="info-bar">
                    <span class="title2">&nbsp;区间名称：</span>
...
<script>
    ...
    export default {
        ...
        created() {
            this.regionId = this.$route.params.regionId; //接收到的区间id
            //向后端请求单条价格区间记录
            getPriceRegion(this.regionId).then(res => {
                var priceRegion = res.data;
                this.regionName = priceRegion.regionName;
            })
          document.getElementById("tabbar").style.display = "flex";
        },
        methods: {
            back() {
                this.$router.replace('/priceRegion/list')
            },
        },
    }
</script>
