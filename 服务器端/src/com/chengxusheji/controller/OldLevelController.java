...
//OldLevel管理控制层
@Controller
@RequestMapping("/OldLevel")
public class OldLevelController extends BaseController {
	...
	public String add(Model model,HttpServletRequest request) throws Exception {
		model.addAttribute(new OldLevel());
		return "OldLevel_add";
	}

	/*客户端ajax方式提交添加新旧程度信息*/
	@RequestMapping(value = "/add", method = RequestMethod.POST)
	public void add(@Validated OldLevel oldLevel, BindingResult br,
        ...
        oldLevelService.addOldLevel(oldLevel);
        message = "新旧程度添加成功!";
        success = true;
        writeJsonResponse(response, success, message);
	}
	...
	public void list(Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		for(OldLevel oldLevel:oldLevelList) {
			JSONObject jsonOldLevel = oldLevel.getJsonObject();
			jsonArray.put(jsonOldLevel);
		}
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(OldLevel oldLevel:oldLevelList) {
			...
			jsonOldLevel.accumulate("levelId", oldLevel.getLevelId());
			jsonOldLevel.accumulate("levelName", oldLevel.getLevelName());
			jsonArray.put(jsonOldLevel);
		}
	...
	public String frontlist(Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		List<OldLevel> oldLevelList = oldLevelService.queryOldLevel(currentPage);
	    /*计算总的页数和总的记录数*/
	    oldLevelService.queryTotalPageAndRecordNumber();
	    /*获取到总的页码数目*/
	    int totalPage = oldLevelService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = oldLevelService.getRecordNumber();
	    request.setAttribute("oldLevelList",  oldLevelList);
	    request.setAttribute("totalPage", totalPage);
	    request.setAttribute("recordNumber", recordNumber);
	    request.setAttribute("currentPage", currentPage);
		return "OldLevel/oldLevel_frontquery_result"; 
	}
	...
	public String frontshow(@PathVariable Integer levelId,Model model,HttpServletRequest request) throws Exception {
        ...
        request.setAttribute("oldLevel",  oldLevel);
        return "OldLevel/oldLevel_frontshow";
	}
	...
	public void update(@PathVariable Integer levelId,Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		//将要被返回到客户端的对象 
		JSONObject jsonOldLevel = oldLevel.getJsonObject();
		out.println(jsonOldLevel.toString());
		out.flush();
		out.close();
	}
	...
	public void update(@Validated OldLevel oldLevel, BindingResult br,
			Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		String message = "";
    	boolean success = false;
		if (br.hasErrors()) { 
		...
		try {
			oldLevelService.updateOldLevel(oldLevel);
			message = "新旧程度更新成功!";
			success = true;
			writeJsonResponse(response, success, message);
		} catch (Exception e) {
	...
	public void delete(String levelIds,HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException {
    	...
    	boolean success = false;
        try { 
        	int count = oldLevelService.deleteOldLevels(levelIds);
        	success = true;
	...
	public void OutToExcel( Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        List<OldLevel> oldLevelList = oldLevelService.queryOldLevel();
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OldLevel信息记录"; 
        String[] headers = { "新旧程度id","新旧程度名称"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<oldLevelList.size();i++) {
        	OldLevel oldLevel = oldLevelList.get(i); 
        	dataset.add(new String[]{oldLevel.getLevelId() + "",oldLevel.getLevelName()});
        }
        /*
		...
		*/
		...
		try { 
			...
			response.setHeader("Content-disposition","attachment; filename="+"OldLevel.xls");//filename是下载的xls的名，建议最好用英文 
